    ...
    for _, item in df.iterrows():
        ...
        if strict and "time" in target.keys():
            if not pd.isnull(item["教室與上課時間"]):
                ...
                slicePairs = [
                    last_segment[i : i + 2] for i in range(0, len(last_segment), 2)
                ]
...
if __name__ == "__main__":
    temp = select(
        {"time": ["F3", "F4", "M1"], "loc": ["GEN II", "ENG I"], "dep": ["MATH"]}
    )
    ...
    temp = select(
        {"time": ["M3", "M4", "W2"], "loc": ["DELTA"], "dep": ["CS"]}, strict=True
    )
    temp.to_csv("./out4.csv")

    temp = select(
        {"time": ["M3", "M4", "W2"], "loc": ["DELTA"], "dep": ["CS", "EE"]}, strict=True
    )
    temp.to_csv("./out5.csv")

    temp = select({"time": ["T3", "T4", "R4"], "dep": ["MATH"]}, strict=True)
    temp.to_csv("./out6.csv")

    temp = select({"time": ["Ma", "Mb", "Mc"], "loc": ["GEN II", "ENG I"]}, strict=True)
    temp.to_csv("./out7.csv")

    temp = select({"time": ["T3", "T4", "R3", "R4"], "dep": ["CHE"]}, strict=True)
    temp.to_csv("./out8.csv")

    temp = select({"time": ["T3", "T4", "R3", "R4"], "dep": ["CHEM"]}, strict=True)
    temp.to_csv("./out9.csv")

    temp = select(
        {"time": ["T3", "T4", "R3", "R4"], "dep": ["CHE", "CHEM"]}, strict=True
    )
